 ...
 * @author Gabriele Ara
 */
/* TODO: se lo rinomino stato attuale e gestisco tutto da qui? */
public class Cache implements java.io.Serializable {
    /* TODO: se metto tutte observable list mi cambiano dinamicamente? */
    private final ArrayList<GlicemiaRilevata> datiDelGrafo; // array
    private final int[] statistiche;
    private final ArrayList<String> utenti; // array
    ...
    private static void leggiCache() {
        try(
            FileInputStream fis = new FileInputStream(cacheFileName);
            ObjectInputStream ois = new ObjectInputStream(fis);
        ) {
            valoreIniziale = (Cache) ois.readObject();
        } catch (IOException | ClassNotFoundException ex) {
            ex.printStackTrace();
        }
    }

    public static void scriviCache(Cache c) {
        try (
    ...
    public Cache(ArrayList<GlicemiaRilevata> datiDelGrafo,
            ...
            int[] statistiche) {
        this(datiDelGrafo, utenti, utenteAttuale, dataAttuale);
